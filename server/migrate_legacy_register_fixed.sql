-- Fixed migration script to transfer necessary data from legacy register table to new structure
-- This script assumes the legacy register table exists and has the correct column names

-- Step 1: Check if legacy register table exists and show its structure
SELECT 'Legacy register table structure:' as info;
DESCRIBE `register`;

-- Step 2: Create backup of existing register table (if any) with a different name
DROP TABLE IF EXISTS `register_backup_current`;
CREATE TABLE IF NOT EXISTS `register_backup_current` AS SELECT * FROM `register`;
...
-- Optional: Clean up backup table after verification (uncomment if you're sure)
-- DROP TABLE register_backup_current;
